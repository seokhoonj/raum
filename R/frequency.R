...
# to be modified
# get_prevalence <- function(df, group_var, cvd_kcd, cycle = c("year", "month"), cast = TRUE) {
#   assert_class(df, "data.table")
#   has_cols(df, c("id", "gender", "age_band", "kcd", "sdate"))
#   prevalence <- check_cvd_kcd(cvd_kcd = cvd_kcd, kcd = df$kcd)
#   setcolnames(prevalence, cvd_kcd)
#   prevalence <- data.table(df[, .(id, gender, age_band, kcd, sdate)], prevalence)
#   set(prevalence, j = "year", value = year(df$sdate))
#   set(prevalence, j = "month", value = month(df$sdate))
#   group_cols <- match_cols(df, vapply(substitute(group_var), deparse, "character"))
#   prevalence_cols <- c(group_cols, cycle[[1L]])
#   cvd_kcd_cols <- match_cols(prevalence, cvd_kcd)
#   prevalence <- prevalence[, lapply(.SD, sum), .SDcols = cvd_kcd_cols, keyby = prevalence_cols]
#   if (cast) {
#     fml <- formula(paste0(paste0(group_cols, collapse = " + "), " ~ ", cycle[[1L]]))
#     prevalence <- lapply(seq_along(cvd_kcd),
#                          function(x) dcast(prevalence, fml, value.var = cvd_kcd_cols[[x]], fill = 0))
#   } else {
#     prevalence <- lapply(cvd_kcd, function(x) {
#       cols <- c(prevalence_cols, x)
#       data <- prevalence[, ..cols]
#       setnames(data, c(prevalence_cols, "count"))
#     })
#   }
#   names(prevalence) <- cvd_kcd
#   return(prevalence)
# }
